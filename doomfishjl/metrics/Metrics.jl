...
# previously acquired Metrics from a snapshot. This leaves the option open.
Metrics() = Metrics( Counters(), Dict{StatsName, Stats}() )
...
macro collectstats(statsName, body)
    ...
    return :( updateStats!( $metrics, $statsName, @timed $body ) )
end
