...
struct DefaultSpriteImpl <: Sprite
    ...
    function DefaultSpriteImpl(template::SpriteTemplate, name::SpriteName, clock::FrameClock)
...
function DefaultSpriteImpl(snapshot::SpriteSnapshot, frameClock::FrameClock)
    return DefaultSpriteImpl(snapshot.template,
                      snapshot.name,
                      snapshot.frameClock,
...
function render(sprite::DefaultSpriteImpl, shaderProgram::ShaderProgram)
    if !sprite.hidden renderTemplate( sprite.template, getRenderedTexture(sprite), sprite.position, shaderProgram ) end
end


function getCurrentFrame(sprite::DefaultSpriteImpl) ::Int
    if sprite.paused
...
convert(String, sprite::DefaultSpriteImpl) = return "Sprite($(sprite.name))"
...
function getRenderedTexture(sprite::DefaultSpriteImpl) :: Int
    return getCurrentFrame(sprite) % sprite.template.textureCount
end


function getTextureName(sprite::DefaultSpriteImpl, framesAhead::Int)
    return sprite.template.textures[ (getCurrentFrame(sprite) + framesAhead) % sprite.template.textureCount ].name
end


function setLayer!(sprite::DefaultSpriteImpl, layer::Int)
...
function setRepetitions!(sprite::DefaultSpriteImpl, repetitions::Int)
...
getAge(sprite::DefaultSpriteImpl) = return sprite.frameClock.currentFrame - sprite.initialFrame


function setPaused!(sprite::DefaultSpriteImpl, paused::Bool)
...
function setHidden!(sprite::DefaultSpriteImpl, hidden::Bool)
...
function close(sprite::DefaultSpriteImpl)
    # if nothing != sprite.soundSource close(sprite.soundSource) end  TODO: sound
end


uploadCurrentFrame(sprite::DefaultSpriteImpl) = uploadTexture( getRenderedTexture(sprite) )


function getSoundPauseLevel(sprite::DefaultSpriteImpl) :: Int # TODO: sound
...
function setPosition(sprite::DefaultSpriteImpl, position::TextureCoordinate)
    checkArgument( isValidCoordinate( position ), "Out of bounds position $position" )
    sprite.position = position
end


function getSoundRemarks(sprite::DefaultSpriteImpl) :: String
