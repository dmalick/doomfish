...
function horse_hell(imageFile)
    ...
    @time vertexData = getVertexArray(img)
    ...
    window = GLFW.CreateWindow(width, height, "HORSE-HELL")
    ...
    if !(glGetShaderiv(vertexShader, GL_COMPILE_STATUS) == GL_TRUE) error("vert shader did not compile") end

    fragmentShader = glCreateShader(GL_FRAGMENT_SHADER)
    fragmentSource = read("/home/gil/.atom/doomfishjl/shaders/sample.frag", String)
    glShaderSource(fragmentShader, fragmentSource)
    glCompileShader(fragmentShader)
    if !(glGetShaderiv(fragmentShader, GL_COMPILE_STATUS) == GL_TRUE) error("frag shader did not compile") end
